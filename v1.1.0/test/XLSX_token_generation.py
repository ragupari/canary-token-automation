...
INPUT_CSV = "users_input_xlsx.csv"
OUTPUT_CSV = "users_token_xlsx.csv"
...
def process_tokens():
    ...
    with open(INPUT_CSV, 'r') as f:
        ...
        # ADDED: Ensure both columns exist in the output headers
        if 'canary_link' not in fieldnames: fieldnames.append('canary_link')
        if 'file_name' not in fieldnames: fieldnames.append('file_name')

    print(f"Starting. Generating {len(reader)} tokens...")

    for row in reader:
        u_id = row['user_id']
        file_name = f"Salaries_Grade_V_and_Above_{u_id}.xlsx"
        file_path = os.path.join(OUTPUT_DIR, file_name)
        
        # ADDED: Store the filename in the row dictionary immediately
